...
class GenericNamedModelViewSet(viewsets.GenericViewSet):
    ...
    def register_with(cls, router):
        ...
        """
        pieces = cls.get_endpoint_pieces()
        if not pieces:
            return
        ...
        if hasattr(cls, 'router_lookup'):
            new_router = routers.NestedDefaultRouter(router, urlpattern, lookup=cls.router_lookup)
            return new_router
        else:
            return

    @classmethod
    def get_endpoint_pieces(cls):
